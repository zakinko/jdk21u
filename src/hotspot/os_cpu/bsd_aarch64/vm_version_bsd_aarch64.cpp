...
void VM_Version::get_os_cpu_info() {
  // Each system answers a different way.  FreeBSD and OpenBSD report
  // through the aux vector; NetBSD has neither <sys/auxv.h> nor AT_HWCAP,
  // and traps an MRS of the EL1 identification registers -- measured on
  // NetBSD 11.99.7/aarch64, ID_AA64ISAR0_EL1, ID_AA64PFR0_EL1 and
  // MIDR_EL1 all raise SIGILL -- but it hands the whole set over through
  // sysctl machdep.cpuN.cpu_id instead, which is read below.
#if defined(__FreeBSD__) || defined(__OpenBSD__)
